...
private async aplicarPreferenciasUsuario(
  ...
  // NUEVO:
  // Si la búsqueda viene centrada en una característica
  // y no hay subcategoría/especialidad explícitas,
  // no inyectamos preferencias porque pueden sesgar la búsqueda
  // y excluir el resultado correcto.
...
resultados = await this.searchService.search({
  ...
  // ------------------------------------------------------------
  // FALLBACK LOCAL
  // AQUÍ QUEDA TU LÓGICA ACTUAL CASI INTACTA
  // ------------------------------------------------------------
  ...
  //   if (
  //     filtros.promos &&
  //     ['promocion', 'promociones', 'promo', 'promos', 'oferta', 'ofertas', 'descuento', 'descuentos'].includes(qFallbackNormalizada)
  //   ) {
  //     filtros.q = undefined;
  //   }
