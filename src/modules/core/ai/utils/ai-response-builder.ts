export class AIResponseBuilder {
    ...
    static respuestaConResultados(tipo: string, filtros: any, items: any[]) {
      ...
      switch (tipo) {
        case 'especialidad':
          titulo = `Encontramos opciones de ${nombreEspecialidad}${ciudadTxt}.`;
          resumen = `Estas son las alternativas disponibles según tu búsqueda.`;
          break;
  
        case 'subcategoria':
          titulo = `Opciones de ${nombreSubcategoria}${ciudadTxt}.`;
          resumen = `Aquí tienes los resultados más relevantes.`;
          break;
  
        case 'categoria':
          titulo = `Negocios de ${nombreCategoria}${ciudadTxt}.`;
          resumen = `Te mostramos lo más relacionado con lo que solicitaste.`;
          break;
  
        default:
          titulo = `Estas son las opciones que coinciden con tu búsqueda${ciudadTxt}.`;
          resumen = `Ordenamos los resultados para ayudarte a elegir fácilmente.`;
          break;
      }
    ...
    static listado(items: any[]) {
      return items.map((i, idx) => ({
        ...
        promo: i.promo
          ? `Promoción disponible: ${i.promo.titulo}`
          : null,
