...
// // permitir búsquedas de promociones sin texto específico
// if (!qRaw && !params.promos) {
...
//   !!params.promos &&
//   (!qNorm ||
//     ['promocion', 'promociones', 'promo', 'promos', 'oferta', 'ofertas', 'descuento', 'descuentos'].includes(qNorm));

// if (!tieneTaxonomia && !esBusquedaGenericaDePromos) {
...
//           .orWhere('v.catalogo_keywords LIKE :t', { t: `%${qNorm}%` })
//           .orWhere('v.anuncio_keywords LIKE :t', { t: `%${qNorm}%` })
//           .orWhere('v.caracteristicas_keywords LIKE :t', { t: `%${qNorm}%` })
//           .orWhere('v.items_keywords LIKE :t', { t: `%${qNorm}%` })
//           .orWhere('v.items_keywords LIKE :t', { t: `%${qNorm}%` });
//       }),
//     );
//   } else {
...
//                 .orWhere(`v.caracteristicas_keywords LIKE :${p}`)
...
// //este muestra las promociones activas primero las mas cercanas al usuario y dentro de las cercanas las 
// //mas actuales, si no hay ubicación muestra primero las promociones activas y dentro de estas las mas actuales
...
async search(params: {
  ...
  // Aunque exista taxonomía, sí aplicamos filtro textual.
    ...
    qb.andWhere(
      new Brackets((b) => {
        b.where('LOWER(it.nombre) LIKE :q', { q: `%${qNorm}%` })
          .orWhere('LOWER(COALESCE(it.descripcion, "")) LIKE :q', { q: `%${qNorm}%` });
      }),
    );
  } else {
    qb.andWhere(
  ...
  const items = rows.entities.map((e: VistaNegociosCompleta, idx: number) => {
    ...
    return {
      ...
      item: {
        ...
        precioBase: raw.item_precio_base != null ? Number(raw.item_precio_base) : null,
